...
fun main()= runBlocking {   //main thread
    ...
    val msg1:Deferred<String> =async(start =CoroutineStart.LAZY) { functionLazy1()}//child 1  // runs on background coroutine let be c1


    val msg2:Deferred<String> =async(start = CoroutineStart.LAZY) { functionLazy2()}//child 2  // runs on background coroutine let be c2 .so they run in parallel
...
suspend fun functionLazy1():String{
    delay(1000)
    println("Lazy fun 1")
    return "hello"
}
suspend fun functionLazy2(): String {
    delay(1000)
    println("lazy fun 2")
    return "World"
}
